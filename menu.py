...
spacing = 20  # Adjust this value for more or less space between buttons

# Define the button rectangles with spacing
play_button_rect = pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2 - 25, 205, 50)  # Play button
shop_button_rect = pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2 + 25 + spacing, 205, 50)  # Shop button
quit_button_rect = pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2 + 75 + (2 * spacing), 205, 50)  # Quit button
...
while show_menu:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_UP:
                selected_button = (selected_button - 1) % 3  # Wrap around to 2 if going up from 0
            elif event.key == pygame.K_DOWN:
                selected_button = (selected_button + 1) % 3  # Wrap around to 0 if going down from 2
                ...
                if selected_button == 0:  # Play
                    ...
                    main.main()
                    break
                elif selected_button == 1:  # Shop
                    show_menu = False
                    shop.shop_loop()
                    show_menu = True
                elif selected_button == 2:  # Exit
                    pygame.quit()
                    sys.exit()
    ...
    # Render the Shop button
    text = font.render("Store", True, WHITE)
    pygame.draw.rect(screen, BLACK, shop_button_rect, border_radius=10)
    if selected_button == 1:
        pygame.draw.rect(screen, RED, shop_button_rect, border_radius=10, width=4)
    text_rect = text.get_rect()
    text_rect.center = shop_button_rect.center
    screen.blit(text, text_rect)
