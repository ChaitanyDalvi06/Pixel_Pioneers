...
import pygame
import random
from classes.meteorAttack import MeteorAttack
...
def save_player_stats(meteor_angle):
    player_stats = {
        "score": score,
        "hi_score": hi_score,
        "angle": meteor_angle,
...
# Ensure stats are saved when the game exits
import atexit
atexit.register(save_player_stats)
...
def load_player_stats():
    if os.path.exists('player_stats.json'):
        ...
        return {"score": 0, "hi_score": 0}

player_stats = load_player_stats()
score = 0
hi_score = player_stats["hi_score"]
...
MeteorAttack_group = pygame.sprite.Group()
...
meteorAttack_start = (1, 0)


player_grid_x = player.rect.x
player_grid_y = player.rect.y
player_grid = (player_grid_y, player_grid_x)


def calculate_direction(start, target):
    dx = target[1] - start[1]  # Difference in x
    dy = target[0] - start[0]  # Difference in y
    distance = (dx ** 2 + dy ** 2) ** 0.5  # Euclidean distance
    if distance == 0:  # Prevent division by zero
        return (0, 0)
    meteorAttack_direction = (dx / distance, dy / distance)
    return meteorAttack_direction

meteorAttack_direction = calculate_direction(meteorAttack_start, player_grid)

meteorAttack_position = [meteorAttack_start[1] * TILE_SIZE, meteorAttack_start[0] * TILE_SIZE]


def update_meteor(meteorAttack_position, meteorAttack_direction, speed):
    meteorAttack_position[0] += meteorAttack_direction[0] * speed
    meteorAttack_position[1] += meteorAttack_direction[1] * speed
    return meteorAttack_position


meteorAttack_image = pygame.image.load("images/explosion/explosion1.png")
meteor_attack = MeteorAttack(start_x=1, start_y=0, target_x=player_grid_x, target_y=player_grid_y, image=meteorAttack_image)
...
shieldTime = 0
last_shield_update = pygame.time.get_ticks()
...
while running:
    ...
    if shieldActivated:
        player.image = pygame.image.load('images/player_shield.png').convert_alpha()
        # player.original_image = player.image.copy()

        if current_time - shieldTime >= 1000:
            shieldTime -= 1000
            last_shield_update = current_time
    ...
    if score >= 50 and random.randint(0, 40) == 0 and len(enemy2_group) < 2:
    ...
    if score >= 55 and not boss1_spawned:
    ...
    if score >= 75 and not boss2_spawned:
        pygame.mixer.Sound('game_sounds/warning.mp3').play()
        boss2_img = boss2_img
        boss2_object = Boss2(
            random.randint(200, WIDTH - 100),
    ...
    if score >= 105 and not boss3_spawned:
        pygame.mixer.Sound('game_sounds/warning.mp3').play()
        boss3_img = boss3_img
        boss3_object = Boss3(
            random.randint(200, WIDTH - 100),
    ...
    if score > 50 and random.randint(0, 100) == 0:
    ...
    if score > 50 and random.randint(0, 500) == 0:
    ...
    if score > 30 and random.randint(0, 100) == 0:
        target_x=player.rect.x
        target_y=player.rect.y
        meteor_attack=MeteorAttack(start_x=1*TILE_SIZE, start_y=0, target_x=target_x, target_y=target_y, image=meteorAttack_image)
        save_player_stats(meteor_attack.angle)
        MeteorAttack_group.add(meteor_attack)
    
    MeteorAttack_group.update()

    if player_life <= 0:
        if show_game_over(score) == "exit":
            running = False
        else:
            player_life = 200
            bullet_counter = 200
    ...
    for black_hole_object in black_hole_group:
        ...
        if black_hole_object.rect.colliderect(player.rect) and not shieldActivated:
            player_life -= 1
    ...
    for meteor_object in meteor_group:
        ...
        if meteor_object.rect.colliderect(player.rect) and not shieldActivated:
            player_life -= 10
    ...
    for meteor2_object in meteor2_group:
        ...
        if meteor2_object.rect.colliderect(player.rect) and not shieldActivated:
            player_life -= 10
    ...
    for meteor in MeteorAttack_group:
        meteor.draw(screen)
    
    for meteor in MeteorAttack_group:
        if meteor.rect.colliderect(player.rect) and not shieldActivated:
            player_life -= 20
            meteor.kill()
    ...
    for enemy_object in enemy1_group:
        ...
        if enemy_object.rect.colliderect(player.rect) and not shieldActivated:
            player_life -= 10
    ...
    for enemy2_object in enemy2_group:
        ...
        if enemy2_object.rect.colliderect(player.rect) and not shieldActivated:
            player_life -= 10
        ...
        for enemy2_bullet in enemy2_bullets:
            if enemy2_bullet.rect.colliderect(player.rect) and not shieldActivated:
                player_life -= 10
    ...
    for boss1_object in boss1_group:
        ...
        if boss1_object.rect.colliderect(player.rect) and not shieldActivated:
            player_life -= 10
        ...
        for boss1_bullet in boss1_bullets:
            if boss1_bullet.rect.colliderect(player.rect) and not shieldActivated:
                player_life -= 10
                explosion = Explosion(player.rect.center, explosion3_images)
                explosions.add(explosion)
                boss1_bullet.kill()
    ...
    for boss2_object in boss2_group:
        ...
        if boss2_object.rect.colliderect(player.rect) and not shieldActivated:
            player_life -= 2
        ...
        for boss2_bullet in boss2_bullets:
            if boss2_bullet.rect.colliderect(player.rect) and not shieldActivated:
                player_life -= 5
                explosion = Explosion(player.rect.center, explosion3_images)
                explosions.add(explosion)
                boss2_bullet.kill()
    ...
    for boss3_object in boss3_group:
        ...
        if boss3_object.rect.colliderect(player.rect) and not shieldActivated:
            player_life -= 1
        ...
        for boss3_bullet in boss3_bullets:
            if boss3_bullet.rect.colliderect(player.rect) and not shieldActivated:
                player_life -= 7
                explosion = Explosion(player.rect.center, explosion3_images)
                explosions.add(explosion)
                boss3_bullet.kill()
